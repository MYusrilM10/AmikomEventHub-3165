...
<div class="p-8 bg-gradient-to-br from-slate-50 to-slate-100 min-h-screen">
    ...
    <div class="mb-8">
        ...
        <p class="text-gray-500 text-sm mt-1">Buat kategori event baru untuk sistem AmikomEventHub</p>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-lg border border-gray-100 p-8 max-w-2xl">
        <form action="{{ route('admin.categories.store') }}" method="POST" class="space-y-6">
            ...
            <div>
                ...
                @enderror
                <p class="text-gray-500 text-xs mt-1">Nama kategori akan digunakan untuk filter event dan pencarian</p>
            </div>
            ...
            <div>
                ...
                <select 
                    ...
                    required
                >
                    <option value="" disabled selected>-- Pilih Popularitas --</option>
                    <option value="Trending" @selected(old('popularity') === 'Trending')>Trending</option>
                    <option value="Popular" @selected(old('popularity') === 'Popular')>Popular</option>
                    <option value="New" @selected(old('popularity') === 'New')>New</option>
                </select>
                @error('popularity')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
                <p class="text-gray-500 text-xs mt-1">Pilih badge untuk menampilkan popularitas kategori</p>
            </div>
            ...
            <div>
                ...
                @error('created_at')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
                <p class="text-gray-500 text-xs mt-1">Pilih tanggal kategori dibuat</p>
            </div>
