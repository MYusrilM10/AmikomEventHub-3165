...
<div class="p-8 bg-gradient-to-br from-slate-50 to-slate-100 min-h-screen">
    ...
    <div class="flex justify-between items-center mb-8">
        <div>
            <h2 class="text-3xl font-bold text-gray-800">Manajemen Event</h2>
            <p class="text-gray-500 text-sm mt-1">Kelola semua data event Anda di sini</p>
        </div>
        <a href="{{ route('admin.events.create') }}" class="bg-gradient-to-r from-indigo-600 to-indigo-700 text-white px-6 py-2.5 rounded-lg font-semibold hover:shadow-lg hover:from-indigo-700 hover:to-indigo-800 transition-all duration-300 transform hover:scale-105">+ Tambah Event Baru</a>
    </div>
                ...
                <tbody class="divide-y divide-gray-200">
                    @forelse($events as $event)
                        <tr class="hover:bg-gradient-to-r hover:from-indigo-50 hover:to-blue-50 transition-colors duration-200 group">
                            ...
                            <td class="px-6 py-4">
                                <div class="text-gray-700 font-medium">{{ \Carbon\Carbon::parse($event->date)->format('d M Y') }}</div>
                                <p class="text-sm text-gray-500">{{ \Carbon\Carbon::parse($event->date)->format('H:i') }}</p>
                            </td>
                            <td class="px-6 py-4">
                                <span class="text-gray-800 font-medium">Rp {{ number_format($event->price, 0, ',', '.') }}</span>
                            </td>
                            <td class="px-6 py-4">
                                <span class="text-gray-700 font-medium">{{ $event->stock }} item</span>
                            </td>
                            <td class="px-6 py-4">
                                <div class="flex gap-2 justify-center">
                                    <a href="{{ route('admin.events.edit', $event->id) }}" class="inline-flex items-center gap-2 bg-blue-600 text-white px-4 py-2 rounded-lg text-sm font-semibold hover:bg-blue-700 hover:shadow-md transition-all duration-200 transform hover:scale-105">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></path></svg>
                                        Edit
                                    </a>
                                    <form action="{{ route('admin.events.destroy', $event->id) }}" method="POST" class="inline" onsubmit="return confirm('Anda yakin ingin menghapus data acara ini secara permanen?');">
                                        @csrf
                                        @method('DELETE')
